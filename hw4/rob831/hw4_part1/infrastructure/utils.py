...
# TODO: get this from previous HW
    ...
    while True:
        ...
        is_rollout_done = int(done or steps >= max_path_length)
...
def sample_trajectories(env, policy, min_timesteps_per_batch, max_path_length, render=False):
    ...
    timesteps_in_this_batch = 0
    paths = []

    while timesteps_in_this_batch < min_timesteps_per_batch:
        this_path = sample_trajectory(env, policy, max_path_length, render)
        paths.append(this_path)
        timesteps_in_this_batch += get_pathlength(this_path)

    return paths, timesteps_in_this_batch
...
def sample_n_trajectories(env, policy, ntraj, max_path_length, render=False):
    ...
    for t in range(ntraj):
        this_path = sample_trajectory(env, policy, max_path_length, render)
        sampled_paths.append(this_path)
